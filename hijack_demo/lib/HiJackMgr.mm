...
-(void)viewDidLayoutSubviews {
    ...
    CGFloat button_w = self.view.frame.size.width/8;
    for (int  i = 0; i < 15; i++) {
        
        UIButton *button = [[UIButton alloc] init];
        if (i <= 7) {
           
            button.frame = CGRectMake(button_w*i + 5, 10, button_w - 5, 50);
        } else {
            
             button.frame = CGRectMake(button_w*(i-8) + 5, 50 + 15, button_w - 5, 50);
        }
...
//		return 1;
//	}
   [self.searilQueue addOperation:[self sendByteMessage:data]];
}
